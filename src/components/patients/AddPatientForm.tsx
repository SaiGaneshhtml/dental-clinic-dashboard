...
const AddPatientForm: React.FC<AddPatientFormProps> = ({ onAdd, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onAdd({ name, dob, contact, healthInfo });
    onClose();
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end space-x-4">
        ...
        <button type="submit" className="px-4 py-2 text-white bg-indigo-600 rounded-md">Add Patient</button>
      </div>
    </form>
  );
};
